...
def add_content(runtime: Runtime, options: dict[str, str | list[str]]) -> bool:
    ...
    def cleanup(_m: Match) -> str:
        ...
        print(f"Cleanup: {str(_m.group(1))} in {cleanup_ids}?")
    ...
    for m in insert_pattern.finditer(old_text):
        insert_text: str = str(runtime.run(m.group(2)))
        print(f"BM: {m.group(1)} [{insert_text[0:5]}]: [{insert_text}]")
        ...
        if pos < 0:
            ...
            continue
        print(f"Pos: {last_pos} + {pos}, Start: {m.start()} End: {m.end()}")
    ...
    new_text = cleanup_pattern.sub(cleanup, new_text)
